...
import ProductForm from '../../components/betForm/betForm'
import Loading from '../../components/loading'

import useBeter from '../../hooks/useBeter/useBeter'
...
const convertISOTimeToTimeOfExpiration = isotime => {
  ...
  return (
    < Center >
      <VStack>
        <Heading color={'gray.800'} lineHeight={1.4} fontSize={{ base: '2xl', sm: '3xl', md: '4xl' }} textAlign="center" px={{ base: '4', sm: '6', md: '12'}}>
          Apuesta y Gana
        </Heading>
        ...
        {active ?
          <>
            <Text  fontSize={{ base: "md", sm: "lg", md: "xl" }} color={"gray.600"} pt={4} py={2}
              fontWeight={400}>Quieres Apostar?</Text>
            <Button background={'#67E992'} color={'white'} w={{ base: "100%", sm: "30%" }} fontSize={{ base: "md", md: "lg" }} colorScheme={"green"} _hover={{ boxShadow: 'xl', }}
              onClick={onOpen}>
              Apostar
            </Button>
            {loadingCoupons ?<Loading />:
              <>
                Acá puedo poner las mejore apuestas vigentes Apuestas
              </>
            }</>
            :
            <Button colorScheme={"green"} color={'white'} w={{ base: "100%", sm: "50%" }} fontSize={{ base: "md", md: "lg" }}_hover={{ boxShadow: 'xl', }}
            onClick={connect}>
              Conecta tu Wallet !
            </Button>
        }
